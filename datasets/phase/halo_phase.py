...
class PhaseDataset_Halo(Dataset):
    ...
    def _read_image(self, path):
        if 'masks' in path:
            # For mask files (assuming .npz or image format)
            if path.endswith('.npz'):
                # Load numpy compressed file
                mask = np.load(path)
                return mask['arr_0'] if 'arr_0' in mask else mask
            else:
                # Read mask as image
                return cv2.imread(path)
        
        # Check if the path contains 'depth'
        elif 'depth' in path:
            try:
                if path.endswith('.npz'):
                    depth = np.load(path)
                    return depth['arr_0'] if 'arr_0' in depth else depth
        ...
        # Default: read as RGB image
            ...
            try:
                ...
                print(f"Error reading RGB image: {e} {path}")
                return None
    ...
    def _video_batch_loader_for_key_frames(self, duration, timestamp, video_id, index, cut_black):
        ...
        for num, image_index in enumerate(sampled_list):
            try:
                ...
                rgb_base_path = '/mnt/hdd1/han/key_frame/data'
                phase_label = self.dataset_samples[image_index]["phase_gt"]
